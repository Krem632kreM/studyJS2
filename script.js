...
let isNumber = function(n) {
    return !isNaN(parseFloat(n)) && isFinite(n) && n!=""/*&& n.indexOf(' ') >= 0*/
};
    ...
    asking: function() {
        ...
        if (confirm('Есть ли у вас дополнительный источник заработка?')) {
            ...
            let cashIncome;
                ...
                } while (isNumber(itemIncome) || itemIncome === null || itemIncome.indexOf(' ') >= 0 )
        ...
        let addExpenses;
        do {
            addExpenses = prompt('Перечислите возможные расходы за рассчитываемый период через запятую', 'инт,комм,проезд');
        } while (isNumber(addExpenses) || addExpenses === null || addExpenses.indexOf(' ') >= 0 || addExpenses === "");
        ...
        for (let i=0; i<2;i++) {
            ...
            } while (!isNaN(property) || property === null  || property.indexOf(' ') >= 0);
